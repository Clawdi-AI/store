...
Generate cover images for WeChat Official Account articles.

**Two modes — choose based on user request:**
- **Placeholder / 占位图** (`--no-ai`): Free random image from Picsum. **No API key required.** Use immediately when user says "占位图", "placeholder", "随机封面", or "先用随机图".
- **AI cover** (default): Requires `image_api_key`. If not configured, ask user to set it up or switch to placeholder mode.

When user asks for a placeholder cover, run the script with `--no-ai` directly — do NOT ask for an API key.
...
**On `[NO_PREVIEW]` lines:** Tell the user: "Cover generated successfully but chat preview isn't available (upload service unreachable). If you publish via API (`wechat-api.ts`), the cover will upload correctly. If you publish via browser, you may need to upload the cover manually in the WeChat editor."
